...
namespace KoiCare_DAOs
{
    public class WaterDAO
    {
        ...
        {
            return _context.WaterParametersTbls.Include(w => w.Pond).ToList();
        }
        public WaterParametersTbl GetParameterById(int id)
        {
            return _context.WaterParametersTbls.Include(w => w.Pond).Where(w => w.ParameterId == id).SingleOrDefault();
        }
